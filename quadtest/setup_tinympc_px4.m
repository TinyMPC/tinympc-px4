...
try
    ...
    status.patchedHoverTrajectoryOutput = patchHoverTrajectoryOutputIfNeeded('quadtest');
...
if patched
    fprintf('[ok] Patched TinyMPC controller block for hover TrajectorySetpoint output.\n');
end
end
...
script = sprintf([...
    'function traj_sp = TinyMPC_Controller(x, xref)\n' ...
    '%%#codegen\n' ...
    '%% TinyMPC hover outer loop (guidance MPC).\n' ...
    '%%\n' ...
    '%% x is [x y z roll pitch yaw vx vy vz p q r] in PX4 NED/local frame.\n' ...
    '%% The hover reference is latched 1 m above the state at the first\n' ...
    '%% step after the app engages, so it is immune to the EKF z-origin\n' ...
    '%% drift that accumulates while grounded.\n' ...
    '%%\n' ...
    '%% MPC_Step_Plan returns u0 = [ax ay az yawspeed] plus the one-step-\n' ...
    '%% ahead predicted state x1. We publish the MPC PLAN (x1 position and\n' ...
    '%% velocity, u0 acceleration feed-forward, x1 yaw, u0 yawspeed) as\n' ...
    '%% [position(1x3), velocity(1x3), acceleration(1x3), yaw, yawspeed],\n' ...
    '%% so PX4''s position controller tracks TinyMPC''s trajectory while\n' ...
    '%% keeping acceleration-to-attitude conversion, attitude control,\n' ...
    '%% rate control, and the mixer downstream.\n' ...
    '\n' ...
    'persistent ref_pos ref_yaw have_ref last_traj\n' ...
    'if isempty(have_ref)\n' ...
    '    have_ref = false;\n' ...
    '    ref_pos = zeros(3,1,''single'');\n' ...
    '    ref_yaw = single(0.0);\n' ...
    ...
    'xref_in = single(xref);\n' ...
    '\n' ...
    'if ~have_ref && all(isfinite(x_single(1:3)))\n' ...
    '    ref_pos = [x_single(1); x_single(2); x_single(3) - single(1.0)];\n' ...
    '    ref_yaw = x_single(6);\n' ...
    '    last_traj = [x_single(1), x_single(2), x_single(3), ...\n' ...
    ...
    'end\n' ...
    '\n' ...
    'hover_ref = zeros(12,1,''single'');\n' ...
    'hover_ref(1:3) = ref_pos;\n' ...
    'hover_ref(6) = ref_yaw;\n' ...
    '\n' ...
    '%% Keep xref connected for code-generation shape inference and future\n' ...
    '%% external trajectory mode, but force this first demo to a 1 m hover.\n' ...
    'if false\n' ...
    '    hover_ref = xref_in;\n' ...
    'end\n' ...
    '\n' ...
    ...
    '\n' ...
    'coder.ceval(''MPC_Step_Plan'', ...\n' ...
    '            coder.rref(x_single), ...\n' ...
    '            coder.rref(hover_ref), ...\n' ...
    '            coder.wref(mpc_u), ...\n' ...
    '            coder.wref(x_next));\n' ...
    '\n' ...
    ...
    '\n' ...
    '%% Solver-failure guard: never publish non-finite setpoints; hold the\n' ...
    '%% last good plan instead (PX4 then tracks the last planned point).\n' ...
    'if all(isfinite(traj_single))\n' ...
...
function patched = patchHoverTrajectoryOutputIfNeeded(model)
patched = false;
...
if patched
    fprintf('[ok] Configured PX4 Low Level Controller to publish TrajectorySetpoint hover commands.\n');
end
end
